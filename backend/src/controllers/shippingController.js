...
exports.selectCourier = async (req, res, next) => {
  try {
    ...
    const shipment = await Shipment.findOneAndUpdate(
      { order: orderId, ...tenantFilter(req) },
      { $setOnInsert: { customerId: req.tenantId, order: orderId }, courier: req.body.courier },
...
exports.generateLabel = async (req, res, next) => {
  try {
    ...
    const shipment = await Shipment.findOneAndUpdate(
      { order: orderId, ...tenantFilter(req) },
      {
        $setOnInsert: { customerId: req.tenantId, order: orderId },
        labelUrl: req.body.labelUrl || '/labels/' + orderId + '.pdf',
...
exports.trackShipment = async (req, res, next) => {
  try {
    const shipment = await Shipment.findOne({ ...tenantFilter(req), trackingNo: req.query.trackingNo }).populate('order').lean();
...
exports.updateDeliveryStatus = async (req, res, next) => {
  try {
    ...
    const shipment = await Shipment.findOneAndUpdate(
      { order: req.body.orderId, ...tenantFilter(req) },
      updates,
      { new: true }
    ).populate('order');
    if (!shipment) return res.status(404).json({ success: false, message: 'Shipment not found.' });
